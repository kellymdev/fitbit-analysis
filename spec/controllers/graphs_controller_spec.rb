...
RSpec.describe GraphsController, type: :controller do
  ...
  context 'for sleeps' do
    ...
    describe '#awakening_data' do
      before do
        get :awakening_data, params: { format: :json }
      end

      it 'provides awakenings data from the users sleeps' do
        expected_data = user.sleeps.order(:date).as_json(only: [:date, :number_of_awakenings]).to_json
      end
    end
  end
end
